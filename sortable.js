...
async function loadHeroes() {
    ...
    searchBox.addEventListener('keyup', event => {
        ...
        let filteredHeroes = heroes.filter(hero => hero.name.toLowerCase().includes(characters));
        let oldTable = document.querySelector('tbody');
        ...
        document.querySelectorAll('#heroesTable thead tr th').forEach(e => e.addEventListener('click', sortTable));
    });
    ...
    const headers = [
        ...
        "gender", "height", "weight", "placeOfBirth", "alignment"
    ];
    ...
    for (let i = 0; i < headers.length; i++) {
        header.insertCell(i).outerHTML =`<th data-column=\"${headers[i]}\" data-order=desc>${headers[i]}</th>`;
    };
